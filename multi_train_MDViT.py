...
nohup python -u multi_train_MDViT.py --exp_name test --config_yml Configs/multi_train_local.yml --model MDViT --batch_size 4 --adapt_method Sup --dataset isic2018 PH2 DMF SKD --k_fold 0 > test.out 2>&1 &
aux losses don't optimize DA
'''
import argparse
...
def main(config):
    ...
    # prepare model
    if config.model == 'MDViT':
        from Models.Transformer.mdvit import MDViT
        model = MDViT(img_size=config.data.img_size, drop_rate=0.1, drop_path_rate=0.1,
        conv_norm=nn.BatchNorm2d, adapt_method=config.model_adapt.adapt_method, num_domains=K, decoder_name='MLPFM')
    elif config.model == 'MDViT_DSN':
        from Models.Transformer.mdvit import MDViT_DSN
        model = MDViT_DSN(img_size=config.data.img_size, drop_rate=0.1, drop_path_rate=0.1,
        conv_norm=nn.BatchNorm2d, adapt_method=config.model_adapt.adapt_method, num_domains=K, decoder_name='MLPFM')       
