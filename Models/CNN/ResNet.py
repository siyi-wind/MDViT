...
class CNNAdapter(nn.Module):
    def __init__(self,in_ch,out_ch,ratio=0.5,stride=1,act_layer=nn.ReLU):
        super().__init__()
        # hidden = int(in_ch*ratio)
        # self.D_fc1 = nn.Conv2d(in_ch,hidden,1,stride,0)
        # self.act = act_layer(inplace=True)
        self.D_fc2 = nn.Conv2d(in_ch,out_ch,1,stride,0)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, adapt_method=None):
        ...
        self.stride = stride
        if adapt_method == 'ParaCNN':
            self.adapter1 = CNNAdapter(inplanes,planes,stride=stride)
            self.adapter2 = CNNAdapter(planes,planes)

    def forward(self, x):
        residual = x

        if self.adapt_method=='ParaCNN':
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, adapt_method=None):
        ...
        self.stride = stride
        if self.adapt_method=='ParaCNN':
            self.adapter1 = CNNAdapter(inplanes,planes)
            self.adapter2 = CNNAdapter(planes,planes,stride=stride)
            self.adapter3 = CNNAdapter(planes,planes*4)
    ...
    def forward(self, x):
        ...
        if self.adapt_method=='ParaCNN':
            out = self.conv3(out)+self.adapter3(out)
        else:
            out = self.conv3(out)
        out = self.bn3(out)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, out_indices=[-1], num_classes=1000, adapt_method=None,drop_rate=0.1):
        ...
        if adapt_method=='ParaCNN':
            self.adapter1 = CNNAdapter(3,64,ratio=1,stride=2)
    ...
    def forward(self, x):
        outs = []
        if self.adapt_method=='ParaCNN':
            x = self.conv1(x)+self.adapter1(x)
        else:
            x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
if __name__=='__main__':
    model = resnet34(pretrained=True, out_indices=[1], adapt_method=None)  # 11m
