...
# from Models.Transformer.Vit import AttentionSup
...
class Attention_Sup(nn.Module):
    ...
    def forward(self, x, domain_label):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)  (B,H,N,K)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)  # (B,H,N,N)

        # TODO domain attention
        x =  attn @ v # (B,H,N,K)
