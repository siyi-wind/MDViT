...
from Models.Decoders import UnetDecodingBlock, UnetDecodingBlockTransformer, MLPDecoder, MLPDecoderFM, DeepLabV3Decoder
...
class SerialBlock_adapt(nn.Module):
    ...
    def forward(self, x, size: Tuple[int, int], domain_label=None):
        ...
        if domain_label != None :
            cur, domain_att = self.factoratt_crpe(cur, size, domain_label)
        else:
            cur, domain_att = self.factoratt_crpe(cur, size)
...
class MHSA_stage_adapt(nn.Module):
    ...
    def forward(self, input, H, W, domain_label=None, rec_att=False):
        for i, blk in enumerate(self.mhca_blks):
            input, domain_att = blk(input, size=(H,W)) if domain_label==None else blk(input, (H,W), domain_label)
        if rec_att == True:
            return input, domain_att
        else:
            return input
...
class FAT_Transformer_adapt(nn.Module):
    ...
    def forward(self, x, domain_label=None):
        ...
        for idx in range(self.num_stages):
            ...
            x, domain_att = self.mhsa_stages[idx](x, H, W) if domain_label==None else self.mhsa_stages[idx](x, H, W,domain_label)
            x = rearrange(x, 'b (w h) c -> b c w h', w=W, h=H).contiguous()
            out.append(x)
        
        return out, domain_att
...
class UFAT_adapt(nn.Module):
    ...
    def forward(self,x,domain_label=None,out_feat=False,):
        ...
        # [(48,128),(96,64),(192,32),(384,16)]
        B = x.shape[0]
        encoder_outs, domain_att = self.encoder(x) if domain_label==None else self.encoder(x,domain_label)
...
class FATNet_adapt(nn.Module):
    ...
    def forward(self, x, domain_label=None, out_feat=False, out_seg=True):
        ...
        for idx in range(self.num_stages):
            ...
            x, domain_att = self.mhsa_stages[idx](x, H, W) if domain_label==None else self.mhsa_stages[idx](x, H, W,domain_label,True)
        ...
        if out_seg == False:
            ...
            return {'seg': None, 'feat': x_d, 'domain_att': domain_att}
        ...
        if out_feat:
            x = nn.functional.adaptive_avg_pool2d(encoder_outs[3],1).reshape(B, -1)
            if self.feature_dim == 15:
                return {'seg': out, 'feat': x, 'pred': self.proj_head(x), 'domain_att': domain_att}
            if self.do_detach:
                x_d = x.detach()
            else: x_d = x
            x_d = self.proj_head(x_d)
            return {'seg': out, 'feat': x_d, 'domain_att': domain_att}
        else:
            return out
...
class FATNet_KT_adapt(nn.Module):
    ...
    def forward(self, x, domain_label=None, d=None, out_feat=False, out_seg=True):
        ...
        if out_seg == False:
            x = nn.functional.adaptive_avg_pool2d(encoder_outs[3],1).reshape(B, -1)
            return {'seg': None, 'feat': x, 'domain_att': domain_att}
...
if __name__ == '__main__':
    ...
    y = model(x, domain_label, out_feat=True, out_seg=False)
    # print(y['seg'].shape)
    # print(y['feat'].shape)
    print(y['domain_att'].shape)
