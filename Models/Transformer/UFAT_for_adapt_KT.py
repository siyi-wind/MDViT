...
from Models.Decoders import UnetDecodingBlock, UnetDecodingBlockTransformer, MLPDecoder, DeepLabV3Decoder, MLPDecoderFM, UnetDecodingBlockTransformer_M
from Models.CNN.MS_Net import MSNetDecoder
...
class FATNet_KT_adapt(nn.Module):
    '''
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(FATNet_KT_adapt, self).__init__()
        self.num_stages = num_stages
    ...
    def forward(self, x, domain_label=None, d=None, out_feat=False, out_seg=True):
        ...
        if self.do_detach:
            for i in range(len(encoder_outs)):
                new_encoder_outs.append(encoder_outs[i].detach())
        else:
            new_encoder_outs = encoder_outs
        # TODO for deeplabv3decoder
        # new_encoder_outs = new_encoder_outs[3]
        
        if self.decoder_name == 'MLPFM':
            new_encoder_outs = new_encoder_outs+decoder_outs
        ...
        if self.decoder_name == 'Transformer':
            ...
            if d == '0':
                aux_out = self.debranch1(new_encoder_outs, img_size=img_size)
            elif d == '1':
                aux_out = self.debranch2(new_encoder_outs, img_size=img_size)
            elif d == '2':
                aux_out = self.debranch3(new_encoder_outs, img_size=img_size)
            elif d == '3':
                aux_out = self.debranch4(new_encoder_outs, img_size=img_size)
            else:
                aux_out = None
...
# 4 features into decoder
class FATNet_ENKT_adapt(nn.Module):
    '''
    several decoders to do knowledge distillation
    A Conv Position encoding + Factorized attention Transformer
    use transformer encoder and decoder
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(FATNet_ENKT_adapt, self).__init__()
        self.num_stages = num_stages
        self.do_detach = do_detach
        self.decoder_name = decoder_name
        ...
        if self.ensemble_method == 'att':
            self.avg_pool = nn.AdaptiveAvgPool2d((1,1))
            self.proj_uni = nn.Linear(embed_dims[0],64)
            self.proj_debranch = nn.Linear(512,64)
    ...
    def forward(self, x, domain_label=None, d=None, out_feat=False, out_seg=True):
        ...
        aux_outs = []
        aux_feat_list = []
        for f in [self.debranch1,self.debranch2,self.debranch3,self.debranch4]:
            if self.ensemble_method == 'att':
                outs = f(new_encoder_outs,img_size=img_size, out_feat=True)
                aux_outs.append(outs['seg'])
                aux_feat_list.append(outs['feat'].reshape((B,-1)))
            else:
                aux_outs.append(f(new_encoder_outs, img_size=img_size))
        
        if self.ensemble_method == 'att':
            aux_feat = torch.stack(aux_feat_list, dim=1)
            # aux_feat = torch.stack(aux_feat_list, dim=1).detach() # (B,4,dim)
            uni_feat = decoder_outs[0].detach()
            # uni_feat = decoder_outs[0].detach()  # (B,64,H,W)
            uni_feat = self.avg_pool(uni_feat).reshape((B,-1))  # (B,64)

            aux_feat = self.proj_debranch(aux_feat)  # (B,4,64)
            uni_feat = self.proj_uni(uni_feat).unsqueeze(2)  # (B,64,1)
            att = torch.matmul(aux_feat,uni_feat).squeeze(-1)  # (B,4)
            att = torch.softmax(att,dim=1)
            if out_feat:
                x = nn.functional.adaptive_avg_pool2d(encoder_outs[3],1).reshape(B, -1)
                return {'seg': [out]+aux_outs, 'feat': x, 'weight': att}
            else:
                return {'seg':[out]+aux_outs, 'weight': att}

    
        if out_feat:
            x = nn.functional.adaptive_avg_pool2d(encoder_outs[3],1).reshape(B, -1)
            return {'seg': [out]+aux_outs, 'feat': x}
        else:
            return [out]+aux_outs
...
class FATNet_KT_adapt_DSN(nn.Module):
    '''
    several decoders to do knowledge distillation
    A Conv Position encoding + Factorized attention Transformer
    use transformer encoder and decoder
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(FATNet_KT_adapt_DSN, self).__init__()
        self.num_stages = num_stages
        self.do_detach = do_detach
        self.decoder_name = decoder_name
...
#         num_stages=4,
#         num_layers=[2, 2, 2, 2],
#         embed_dims=[64, 128, 320, 512, 512],  # [32, 64, 128, 256, 512]
#         mlp_ratios=[8, 8, 4, 4],
#         num_heads=[8, 8, 8, 8],
#         qkv_bias=True,
#         qk_scale=None,
...
#         # bridge 
#         self.bridge = nn.Sequential(
#             nn.Conv2d(embed_dims[3],embed_dims[4],kernel_size=3,stride=1, padding=1),
#             conv_norm(embed_dims[4]),
#             nn.ReLU(inplace=True),
#             nn.Conv2d(embed_dims[4],embed_dims[4],kernel_size=3,stride=1, padding=1),
#             conv_norm(embed_dims[4]),  # TODO from embed_dims[3]*2 change to embed_dims[3]
#             nn.ReLU(inplace=True)
#         )
...
#         self.decoder1 = UnetDecodingBlockTransformer(embed_dims[4],embed_dims[3],self.mhsa_list[3],conv_norm=conv_norm)  # 768,384
#         self.decoder2 = UnetDecodingBlockTransformer(embed_dims[3],embed_dims[2],self.mhsa_list[2],conv_norm=conv_norm)  # 384,192
#         self.decoder3 = UnetDecodingBlockTransformer(embed_dims[2],embed_dims[1],self.mhsa_list[1],conv_norm=conv_norm)   # 192,96
#         self.decoder4 = UnetDecodingBlockTransformer(embed_dims[1],embed_dims[0],self.mhsa_list[0],conv_norm=conv_norm)    # 96,48
#         self.finalconv = nn.Sequential(
...
#         # 
#         # MS Decoder   resunet
#         self.debranch1 = MSNetDecoder(in_chans,1,embed_dims,1)
#         self.debranch2 = MSNetDecoder(in_chans,1,embed_dims,1)
#         self.debranch3 = MSNetDecoder(in_chans,1,embed_dims,1)
#         self.debranch4 = MSNetDecoder(in_chans,1,embed_dims,1)
...
if __name__ == '__main__':
    ...
    model = FATNet_KT_adapt_DSN(adapt_method='Sup',num_domains=4, decoder_name='MLPFM')
    # model = FATNet_ENKT_adapt(adapt_method='Sup',num_domains=4, decoder_name='MLP', ensemble_method='att')
