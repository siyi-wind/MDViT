...
class CoaTSeg(nn.Module):
    ...
    def __init__(self, 
        ...
        super(CoaTSeg, self).__init__()
        self.pretrained = pretrained
        self.out_features = ['x1_nocls','x2_nocls','x3_nocls','x4_nocls']
        # encoder
        coat_list = [coat_lite_tiny, coat_lite_mini, coat_lite_small, coat_lite_medium]
        self.encoder = coat_list[coat_index](return_interm_layers=True, 
            out_features=self.out_features, drop_rate=drop_rate, drop_path_rate=drop_path_rate)

        if self.pretrained == True:
            pre_s_dict = torch.utils.model_zoo.load_url(model_urls['coat_lite_mini'])
            self.encoder = load_pretrain(self.encoder, pre_s_dict)
        ...
        # decoder
        self.decoder1 = UnetDecodingBlock(embed_dims[3]*2,embed_dims[3])  # 768,384
        self.decoder2 = UnetDecodingBlock(embed_dims[3],embed_dims[2])  # 384,192
        self.decoder3 = UnetDecodingBlock(embed_dims[2],embed_dims[1])   # 192,96
        self.decoder4 = UnetDecodingBlock(embed_dims[1],embed_dims[0])    # 96,48
    ...
    def forward(self,x):
        ...
        # bridge
        out = self.bridge(encoder_outs[self.out_features[3]])

        # decoding
        out = self.decoder1(out, encoder_outs[self.out_features[3]])  # (384,16,16)
        out = self.decoder2(out, encoder_outs[self.out_features[2]])  # (192,32,32)
        out = self.decoder3(out, encoder_outs[self.out_features[1]])  # (96,64,64)
        out = self.decoder4(out, encoder_outs[self.out_features[0]])  # (48,128,128)
