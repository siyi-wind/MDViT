...
class Block(nn.Module):
    def __init__(self, dim, num_frames, num_heads, mlp_ratio=4., scale=0.5, num_tadapter=1, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        elif adapt_method == 'ParaMLP':
            self.adapter1 = nn.ModuleList([Adapter(dim,skip_connect=True) for _ in range(num_domains)])
            self.adapter2 = nn.ModuleList([Adapter(dim,skip_connect=False) for _ in range(num_domains)])       
...
#         self.relu1 = nn.ReLU(inplace=True)
#         self.conv2 = nn.Conv2d(in_ch//2,in_ch//2,3,1,1)
#         self.bn2 = nn.BatchNorm2d(in_ch//2)
#         self.relu2 = nn.ReLU(inplace=True)
#         self.conv3 = nn.Conv2d(in_ch//2,out_ch,3,1,1)
...
if __name__ == '__main__':
    model = ViTSeg_CNNprompt_adapt4(adapt_method=False,num_domains=1,pretrained=True)
