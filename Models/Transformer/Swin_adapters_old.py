...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.adapter1 = Adapter(dim,skip_connect=True)
        self.adapter2 = Adapter(dim,skip_connect=True)
    ...
    def forward(self, x):
        ...
        # TODO adapter
        attn_windows = self.adapter1(attn_windows)
...
class SwinSeg_adapt(nn.Module):
    ...
    def __init__(
        ...
        conv_norm=nn.BatchNorm2d, num_domains=4,
        **kwargs,):
        ...
        self.bridge = nn.Sequential(
            nn.Conv2d(embed_dims[3],embed_dims[3],kernel_size=1,stride=1, padding=1),
            conv_norm(embed_dims[3]),
            nn.ReLU(inplace=True),
        )

        # decoder
        self.decoder1 = UnetDecodingBlock(embed_dims[3],embed_dims[3],conv_norm=conv_norm)  # 768,384
        ...
        self.init_weights(pretrained_swin_name)
    
    def init_weights(self,pretrained_name):
        def _init_weights(m):
            if isinstance(m, nn.Linear):
                trunc_normal_(m.weight, std=.02)
                if isinstance(m, nn.Linear) and m.bias is not None:
        ...
        if self.pretrained:
            ...
            self.encoder = load_pretrain(self.encoder,pretrained_encoder.state_dict())
...
if __name__ == '__main__':
    model = SwinSeg_adapt(pretrained=False)
