...
from Models.Decoders import UnetDecodingBlock_M
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.norm1 = nn.ModuleList([norm_layer(dim) for _ in range(num_domains)])
        self.attn = WindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = nn.ModuleList([norm_layer(dim) for _ in range(num_domains)])
    ...
    def forward(self, x, d):
        ...
        # TODO adapter
        attn_windows = self.adapter1[int_d](attn_windows)
        ...
        # TODO FFN adapter
        x = x + self.drop_path(self.adapter2[int_d](self.mlp(self.norm2[int_d](x))))
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, num_domains=1):
        ...
        self.norm = nn.ModuleList([norm_layer(4 * dim) for _ in range(num_domains)])
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, num_domains=1):
        ...
        if norm_layer is not None:
            self.norm = nn.ModuleList([norm_layer(embed_dim) for _ in range(num_domains)])
        else:
            self.norm = None
...
if __name__ == '__main__':
    model = SwinSeg_adapt(pretrained=True,adapt_method=False,num_domains=1)
